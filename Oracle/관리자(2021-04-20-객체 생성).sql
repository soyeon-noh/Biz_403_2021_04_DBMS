...
DATAFILE 'C:/oraclexe/data/school.dbf' -- 윈도우에서는 경로를 지정해주는 게 좋다
...
-- 새로운 사용자 계정을 등록할 때 TableSpace와 연결하여 
-- 데이터를 추가할 수 있도록 설정
-- scuser 계정으로 데이터를 추가하면
--      자동으로 schoolDB tablespace에 데이터가 모이게 된다.
...
-- CREATER SESSION : DBMS 서버에 접속(login)을 할수 있는 권한
-- GRANT ** TO user : user에게 ** 권한을 부여한다.
GRANT CREATE SESSION TO scuser; -- 단순히 로그인만할 수 있다.
...
--      세부적으로 관리해야 한다.
--      하지만 학습(실습)하는 입장에서 
--      일일이 세부적으로 지정하는데에 불편함이 있다.
--      때문에 사용등록하고 DBA(DB관리자) 권한을 부여하여
--      실습에 용이하도록 사용하겠다.
GRANT DBA TO scUSER;
