...
-- 데이터가 저장된다.
-- 실제 DBMS 소프트웨어가 저장하는 방식은
--      고유한 기술이 적용되어 있지만
-- DBMS 소프트웨어가 사용하는 사용자는 모든 데이터가
--      표준 형식인 표(TABLE) 형식으로 저장된 것으로 인식한다.
...
-- 데이터 Type 
-- 문자열 데이터
--      CHAR : 저장되는 데이터 길이가 (규칙으로)정해진경우,
--              데이터 길이가 모두 같은 경우
--              (고정길이 문자열)
--      VARCHAR : 저장되는 데이터의 최대크기만 지정하고
--              최대 크기보다 짧은 데이터를 저장할 수 있다.
--              최대 크기보다 짧은 데이터를 저장하면
--              자동으로 공간을 축소하여 데이터를 저장한다.
--              (가변길이 문자열)
--      nVARCHAR : 한글과 같은 UNICODE 문자열을 저장할 때
--              영문알파벳과 저장하는 방식이 달라서
--              문제를 일으킬 수 있다.
--              한글과 같은 UNICODE 데이터를 저장하는 칼럼은
--              nVARCHAR 키워드를 사용한다.
--              // 영어나 한글이나 상관없이 자리수를 셀 수 있다)
--              // 한글이 필요없는 경우엔 사용하지 않는다.

-- 숫자 데이터
--      정수, 실수 등을 구분하여 저장하기도 하지만
--      오라클에서는 NUMBER 키워드로 일괄 지정하여도 상관 없다.
--      저장되는 자리수에 따라 자동으로 크기가 조절된다.
--      
CREATE TABLE tbl_score (
    sc_num CHAR(5), -- 문자열 5자리의 데이터만 저장, String num 같은 것
    sc_name nVARCHAR2(20), -- 최대 20자리이나 데이터 크기에 따라 늘거나준다. String name
    sc_kor NUMBER,
    sc_eng NUMBER,
    sc_meth NUMBER -- 오타났는데 그냥 진행
);

-- DML(Data Manipulation Lang. 데이터 조작어)
--                  (비공식 Data Management Lang. 데이터 관리어)
-- 생성된 TABLE에 데이터추가, 조회, 수정, 삭제를 하는 명령어

-- DBMS 대표적인 CRUD 업무
--   데이터추가(데이터 생성 : Create 업무) : INSERT
--   데이터조회(읽기 등등 : Read 업무) : SELECT
--   데이터변경, 수정 : UPDATE
--   데이터삭제 :DELETE

INSERT INTO tbl_score(sc_num, sc_kor, sc_eng, sc_meth)
...
-- 데이터 읽기
SELECT sc_num, sc_kor, sc_eng, sc_meth -- 오타났는데 그냥 진행
FROM tbl_score;

SELECT sc_name FROM tbl_score;

-- 재앙 명령어, 모든 이름을 전부 임꺽정으로 바꾼다
UPDATE tbl_score
SET sc_name = '임꺽정';
