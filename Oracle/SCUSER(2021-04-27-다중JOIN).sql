...
-- JOIN을 수행하라
-- 학생테이블의 모든테이블을 나열하고 
--학과테이블에서 일차하는 데이터를 가져와서 연관하여 보여라
...
ORDER BY 학번; --학번순정렬, view에서는 AS설정한게 칼럼이기때문
...
-- 3. 학과코드와 학과명으로 그룹을 설정하고
-- 각 그룹의 총점과 평균을 계산
-- 최고점과 최저점 추가
-- 인원수 추가
-- (~~ 별로 는 GROUP BY가 나와야한다)
SELECT 학과코드, 학과명,
        COUNT(*) 인원수,
        SUM(총점) 학과총점, 
        ROUND(AVG(평균),0) AS 학과평균, -- AS는 넣든 안넣든 상관없다
        MAX(평균) 최고점,
        MIN(평균) 최저점
FROM view_성적일람표
GROUP BY 학과코드, 학과명
