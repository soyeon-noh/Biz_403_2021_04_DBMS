...
--      객체 변경(수정) ALTER TABLE, INDEX // VIEW는 거의하지않는다
-- ORACLE 전용 DDL
--      객체 생성 CREATE SEQUENCE

-- 거래내역 저장할 Table
CREATE TABLE tbl_iolist (
    io_date VARCHAR2(10), -- 2021-04-21 이렇게 입력할 것임
...
-- 다시 데이터 INSERT 테스트 (상품 데이터를 입력하지 않은 상태)
INSERT INTO tbl_iolist(io_date, io_buyer, io_qty, io_price)
VALUES ('2021-01-01', '홍길동', 10, 1000);
-- 명령수행에서 오류발생
...
FROM tbl_iolist; -- iolistDB.iouser.tbl_idlost에서 데이터를 가져와라
...
INSERT INTO tbl_iolist(io_date, io_buyer, io_pname, io_qty, io_price)
VALUES ('2021-01-01', '이몽룡', '어묵탕', 10, 2500);

INSERT INTO tbl_iolist(io_date, io_buyer, io_pname, io_qty, io_price)
VALUES ('2021-01-01', '임꺽정', '빗자루', 10, 4000);
...
-- io_buyer, io_pname 칼럼만 보이고 나머지 칼럼은 숨겨서 보여달라
-- 보여지는 칼럼에는 별명을 붙여서 보여라 : AS (별명)

SELECT io_buyer AS 고객이름, io_pname AS 구매상품  -- projection지정. 이 칼럼만 확인하고싶다 라는 뜻.
FROM tbl_iolist
WHERE io_buyer = '홍길동';
