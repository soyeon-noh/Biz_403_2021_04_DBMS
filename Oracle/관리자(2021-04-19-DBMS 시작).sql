-- 여기는 SQL 코드를 작성하고 테스트할 수 있는 화면이다.
-- SQL코드를 작성할 때 '--'로 시작하는 문장은 주석이다.
-- SQL코드는 대소문자 구분이 없다.
...
--      Oracle 이외의 다른 DBMS소프트웨어에서는
--          데이터 저장소를 "DataBase" 라고 한다.
--      생성하기 : CREATE
-- 로컬디스크의 oraclexe/data 폴더에 mydb.dbf라는 파일을 생성하고
-- mydb라는 이름으로 사용하겠다.
-- 데이터가 있든 없든간에 기본 크기를 10MByet로 설정해 달라.
...
--      명령을 실행한 후 결과를 다시 SQL Developer 에게 return
--  다. SQL Developer는 Console에 결과를 보여준다.

-- DROP : CREATE로 생성한 개체(Object, 요소)를 제거하는 명령어
...
-- 2. 사용자 생성하기
-- Oracle DBMS에 접속하여 명령어를 수행할 수 있는 사용자 계정 추가
