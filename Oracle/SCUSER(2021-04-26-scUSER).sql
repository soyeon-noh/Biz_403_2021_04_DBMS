...
-- (AS를 쓰지 않아도 괜찮다. 요즘 DBMS들은 AS안써도 되게 나온다)
...
SELECT sc_num, st_name, st_dept, 
    sc_kor, sc_eng, sc_math
FROM tbl_Score, tbl_student
WHERE sc_num = st_num;
...
-- 문제가 발생할 수 있다.
-- 문제가 발생할 경우에는 각 테이블 일므을 명시해 주어야 한다.
...
-- 학생데이터에는 1 ~ 79번까지만 있고
-- 성적데이터에는 1 ~ 100번까지만 있다.
-- 성적데이터의 80 ~ 100번가지는 
-- 실재 존재하는 학생인지 아닌지 증명할 방법이 없다.
-- 성적데이터는 무결성이 깨진상태가 된다.
-- 학생테이블과 성적테이블간의 연관(관계) 참조가 
-- 무너진 상태가 된다
-- == '참조 무결성 오류가 발생했다' 라고 표현한다.
-- 참조 무결성에 문제가 생긴경우
-- JOIN을 했을때 인출되는 데이터의 신뢰성을 보증할 수 없다.
SELECT * FROM tbl_student; --?? 80이상 삭제됏는데 st_num이 어떻게 존재함?
SELECT * FROM tbl_score;
...
-- ROUND() DMS시스템 함수, 소수점이하 자리수 제한
-- ROUND(값, 자릿수) : 지릿수 이하 반올림
...
-- ORDER BY (AVG(평균),0) DESC, BY SUM(총점) DESC;