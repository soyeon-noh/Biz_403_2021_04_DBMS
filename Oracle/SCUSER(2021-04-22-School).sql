...
CREATE TABLE tbl_student(
    st_num CHAR(5), --고정문자열 5
    st_name nVARCHAR2(20), --한글 가변문자열 20
    st_dept nVARCHAR2(10), --한글 가변문자열 10
    st_grade VARCHAR2(5), --가변문자열 5
    st_tel VARCHAR2(20),
...
-- tbl_student table에 저장되어 있는 모든 데이터를 무조건 보여달라
...
-- 데이터를 추가하려고 할때
-- 이름 데이터가 없어도 데이터가 정상적으로 추가되어버린다.
-- 같은 학번에 데이터가 이미 추가되어 있어도 또다시 추가되어버린다.
-- 이런식으로 데이터가 계속 추가된다면 전체 데이터의 신뢰성에
--  문제가 될 것이다.
-- DBMS에서는 table(Entity)를 설계할 때 이러한 오류를 사전에 방지하기 위해
-- Table을 생성할때 "제약조건"을 설정하여 데이터가 INSERT 되지 못하도록
-- 하는 기능이 있다. 
-- 작성된 table을 삭제하고 다시 제약조건을 설정하여 생성하자

DROP TABLE tbl_student; 
--table 삭제, 테이블은 CASCADE CONSTRAINTS안해도된대
...
-- 반드시 원하는 데이터 1개만 보여주는 조건을 만족하게 하는 칼럼
-- 제약조건이 반드시 UNIQUE하면서 NOUT NULL 이어야 한다. 
-- 기본키는 제약조건에 UNIQUE와 NOT NULL을 같이 설정해야 하는데
-- DBMS에서는 기본키 제약조건을 설정하는 키워드가 별도 있다.
