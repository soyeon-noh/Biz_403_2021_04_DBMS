...
-- 테이블의 구조를 확인하는 명령어
DESC tbl_books; -- tbl_book 테이블을 변경하라
--특정 칼러의 이름을 변경
ALTER TABLE tbl_books RENAME COLUMN bk_author TO  bk_author1; -- bk_authdor 찰럼을 bk_

-- bk_author2라는 칼럼을 생성
-- 한글가변문자열2로 선언하고 NOT NULL로 설정하라
-- ALTER TABLE을 이용하여 칼럼을 추가하는 경우
...
-- (두개의 SELECT 문이 합해진 리스트가 나온다 (연합하다))
UNION ALL 

SELECT '(' || bk_isbn, bk_author2 FROM tbl_books
WHERE bk_author2 IS NOT NULL -- bk_author2 칼럼에 데이터가 있는 경우만 출력
GROUP BY '(' || bk_isbn, bk_author2;

-- 도서정보와 저자리스트를 JOIN하여 데이터 조회
-- 저자가 1명인 경우는 한개의 도서만 출력이 되고
-- 저자가 2명 이상인 경우는 같은 ISBN, 같은 도서명, 다른 저자 형시으로
-- 저자 수 만큼 데이터가 출력된다.
...
-- 저자정보에는 ISBN 저자명을 포함한 데이터를 추가해주면 된다.
...
INSERT INTO tbl_book_author( ba_seq, ba_isbn, ba_author)
VALUES( 37, '9791162540572', '홍길동'); -- 이렇게 SEQ값을 하나하나 해야함..
-- 스트레스. 자동으로만드는 방법이 있다.