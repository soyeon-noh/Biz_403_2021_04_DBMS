...
SELECT st_num, st_dept
FROM tbl_student;

-- projection
-- 보여지는 칼럼의 순서도 바꿀 수 있다.
-- 실제 데이터는 아무변화 없다. view에서만 순서가 바뀔뿐
...
-- 때문에 여기에서 보여지는 데이터는 모두가 LIST이다
-- 데이터가 하나라도 List<VO> voList에 담아야한다.
...
FROM tbl_student
WHERE st_addr LIKE '%북%'
-- ORDER문은 WHERE 문의 가장마지막에 나온대. 
ORDER BY st_addr; -- ASC( 오름차순 ) 가나다순
