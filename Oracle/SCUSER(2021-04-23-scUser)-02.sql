...
-- 확정이 안된 상태에서 
-- ROLLBACK 명령을 수행하면
--      모든 CUI( Insert, Update, Delete)가 취소된다.
-- CUD를 수행하여 데이터에 변화가 발생하면
--      명령을 완료한 후에 반드시 '확정'을 지어야 한다.
--      이때 사용하는 명령이 'COMMIT'
-- COMMIT이 실행되어야만 저장소에 데이터가 확정된다.     

ROLLBACK;

DELETE FROM tbl_score; -- tbl_score에 있는 모든 데이터를 삭제하라
...
-- 재앙 명령어 DELETE, UPDATE
-- WHERE를 꼭 넣어서 전체가 날라가는 일을 방지하자
-- 그리고 WHERE 의 대상은 PK이다. 무결성을 위해서!

-- 데이터의 무결성, 일관성을 유지하고
--      갱신이상, 삭제이상을 방지하는 방법
--
-- DELETE, UPDATE 명령을 수행할때는 
-- 옵션항목인 WERE 절을 반드시 포함하자
-- 또한 특별하게 필요한 경우가 아니면
-- WHERE 절에 포함하는 조건은 PK 칼럼을 기준으로 하자
-- (WHERE 절 없이 사용은 가능하지만 그렇게 쓰지 말자!!!)
...
-- VIEW를 만들때 설정한 AS값들이 칼럼 이름으로 사용된다
SELECT * FROM view_score
WHERE 총점 < 200;
...
-- 총점이 150 이상이고 200 이하인 점수만 보여라
SELECT * FROM view_score
WHERE 총점 >= 150 AND 총점 <= 200;

-- 조건값이 포함된 범위를 조회(검색)할때 ( = 이 들어간 이상, 이하로 봐야한다 )
SELECT * FROM view_score
...
-- 총점(총합), 평균, 최대값, 최소값, 개수를 계산하는 함수

-- 현재 view_score에 들어있는 전체 데이터가 몇개냐
