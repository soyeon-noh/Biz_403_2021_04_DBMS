...
INSERT INTO 
tbl_guest_book(gb_date,
		gb_time, gb_writer,gb_email,
		gb_password, gb_content)
VALUES(
'2021-05-18', '10:29:00', 'soyeon', 
'soyeon@gmail.com', '12345', '오늘은 화요일'); 
...
tbl_guest_book(gb_date,
		...
		gb_password, gb_content)
VALUES(
'2021-05-18', '11:19:00', 'soyeon', 
'soyeon@gmail.com', '12345', '내일은 석가탄신일'); 
...
-- 날짜와 시간기준으로 최근 글이 제일 먼저 보이도록
SELECT * FROM tbl_guest_book
ORDER BY gb_date DESC, gb_time DESC;

-- UPDATE, DELETE 와 같이
-- 2개 이상의 레코드에 영향이 미칠 수 있는 명령은
-- 매우 신중하게 실행해야 한다.
-- 가장 좋은 방법은 
-- 변경, 삭제하고자 하는 데이터가 여러개 있더라도
...
SELECT * FROM tbl_guest_book;
ROLLBACK;
-- 오라클은 UPDATE, DELETE하고 ROLLBACK하면 돌아가는데 
-- MySQL은 안되네.

SELECT 30 * 40;
-- 오라클은 듀얼이라는 범위테이블을 위해 연산이가능했다.
-- MySQL은 SELECT를 사용하여 연산가능
...
-- 오라클의 DECODE()와 유사한 형태의 조건연산
-- MOD를 사용하여 gb_seq 의 값이
--  짝수이면 짝수로 표시, 아니면 홀수로 표시
