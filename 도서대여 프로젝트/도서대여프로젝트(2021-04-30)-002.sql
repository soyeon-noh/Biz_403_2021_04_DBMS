...
-- JOIN된 데이터가 누락되거나 (null)이 출력되는 것은
-- 데이터에 문제가 발생한 것이다.
-- 즉, "참조무결성"이 무너졌다고 한다. 
...
-- JOIN한 결과가 잘못되면 DB 신뢰성에 매우 큰 문제가 발생한다.
...
-- (오류가뜸 parent key not found)

/* 두 테이블은 참조무결성 상태, 
    FOREIGN KEY(외래키)가 설정되어 있다.
=========================================================
    tbl_books.bk_acode            tbl_author.au_code
---------------------------------------------------------
    여기에 코드가 있으면    >>    반드시 있어야 한다
    있을 수 있다            <<    여기에 코드가 있으면
    절대 있을 수 없다       <<    코드가 없으면
    코드가 있으면           >>    삭제 불가능
    코드가 있으면           >>    코드값 변경 불가
