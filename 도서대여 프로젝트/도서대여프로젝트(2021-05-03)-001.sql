...
-- child 테이블에서 parent 테이블의 PK와 연동되는 것

-- tbl_book_rent와 tbl_books, tbl_buyer table을
-- 참조무결성 설정
-- 대상 Table은 다중관계의 tble
-- tbl_buyer 데이터 1개(1명의) 고객이
-- 다수의 tbl_book_rent tble에 포함이 된다
...
tbl_book_rent table에는 필수로 사용하는 데이터 칼럼을
PK로 설정하는데 어려움이 있다.
다른 table의 칼럼을 참조하고 있지만 
...
오라클 이외의 다른 DBNS에는 일련번호를 자동으로
만들어주는 Table 속성이 있다.
오라클(11 이하)에서는 그러한 속성이 없다.

mySQL의 경우 칼럼 속성에 AUTO_INCREMENT로 설정하면
그 칼럼은 INSERT할 때 별도 값을 지정하지 않아도
항상 유일한 일련번호로 자동 생성된다.
...
-- ( 다른 옵션들도 있지만 사용되지 않음. 이것까지만 알아두자)
...
VALUES(seq_book_rent.NEXTVAL, 
        '2021-05-03', '9791162540718', 'B0001');
...
-- tbl_book_rent는 회원ㄴ이 도서를 대여한 리스트가 있다.
...
-- FK 관계가 설정되지 않은 Table 간에서는 죄회는 데이터가
-- 실제와 다를 수 있다.
SELECT 
    BR.br_sdate AS 대여일,
    BR.br_bcode AS 회원코드,
...
FROM tbl_book_rent BR
    -- INNER JOIN 이라고 이너를 앞에 붙여도 되고 아니어도 된다.
    JOIN tbl_books BK
...
-- (옛날에는 left join이 느리다고해서 다른 걸 썻는데 
-- 요즘엔 그냥 키워드 헷갈리면 쓰자. 컴퓨터가 좋아졌기 때문)
...
/*
위의 코드는
전체 데이터중에서 대여일과 반납일에 조건을 부여한 후
데이터를 간추리고, 간추려진 데이터를 GROUP으로 묶어 보여준다.
...
데이터가 많은 수록 성능 차이는 매우 극명하게 나타난다.
*/
...
-- 이 둘은 결과가 같으나 두번째 코드가 효율이 매우 안좋다.



-- 예시) 실행안됨 
/*
    데이터가 아래와같이 제 2정규화가 되어잇다면
    학생이름   과목  점수
    ---------------------
    홍길동     국어  50
    홍길동     영어  50
...
VALUES (seq_book_rent.NEXTVAL,
            '2021-04-01', 'B0011', '9791162540466');
            
INSERT INTO tbl_book_rent(br_seq,
            br_sdate, br_bcode, br_isbn)
VALUES (seq_book_rent.NEXTVAL,
            '2021-04-01', 'B0011', '9791162540527');            
